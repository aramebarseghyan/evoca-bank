...
const Partners = () => {
  ...
  useEffect(() => {
    const fetchPartners = async () => {
      try {
        setLoading(true);
        const docRef = doc(db, "benefits_config", "partners");
        const docSnap = await getDoc(docRef);

        if (docSnap.exists()) {
          setPartners(docSnap.data().items || []);
        }
      } catch (error) {
  ...
  const getLogoSrc = (logo) => {
    ...
    const key = logo.split("/").pop().replace(".png", "").toLowerCase();
    return localLogosMap[key] || logo;
  };
      ...
      <div className="max-w-[1600px] mx-auto flex flex-col xl:flex-row xl:items-center justify-between gap-10 xl:gap-14 2xl:gap-16 relative">
        ...
        <div className="w-full xl:w-[60%] 2xl:w-[65%] relative overflow-visible mt-4 lg:mt-8 xl:mt-0">
          ...
          >
            ...
            <div className="w-full md:pl-[150px] lg:pl-[180px] xl:pl-[200px] 2xl:pl-[220px] relative z-20 min-h-[100px] flex items-center justify-center">
              {loading ? (
                <div className="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-[#580BD4]"></div>
              ) : (
                <Swiper
                  ...
                  className="partners-swiper py-2 px-10 sm:px-16 w-full"
                >
