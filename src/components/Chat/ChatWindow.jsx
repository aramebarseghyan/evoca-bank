...
const ChatWindow = ({
  ...
  if (incomingCall) {
    ...
    const callerUser = allUsers.find(
      (u) => u.uid === finalTargetUid || u.id === finalTargetUid,
    );
    if (callerUser) {
      targetUserName =
        callerUser.displayName || callerUser.email?.split("@")[0] || "Անանուն";
    } else {
  ...
  return (
    <>
      {/* Զանգի մոդալը՝ բարձր z-index-ով (z-[70]), որպեսզի միշտ լինի ամենավերևում */}
